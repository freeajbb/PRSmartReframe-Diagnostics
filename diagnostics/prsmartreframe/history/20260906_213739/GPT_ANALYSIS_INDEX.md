...
- Phase 5B-4A has highest diagnostic priority: verify source reservation SHA, exact baseline binding/digest, selected single abnormality, wouldWrite=false, actual status/reason/failed gate, restore exact-match, case conservation, Unexpected READY=0, Premiere Write Calls=0, Provider Calls=0, and Host Mutation=false. Phase 5B-3 remains frozen upstream evidence only.
